...
public class FetchList {
	...
	private static void getUrls() throws IOException, InterruptedException {
		...
		if (responseCode == HttpURLConnection.HTTP_OK) { // success
			...
			for (int i = 0; i < jsonObjects.length(); i++) { // iteration
				...
				launchAppAndDownloadVideo(tmp.getString("encryptedlink"), tmp.getDouble("duration"), i);
				// driver.get(tmp.getString("encryptedlink"));
				System.out.println("Encryptedlink-" + i + ":" + tmp.getString("encryptedlink"));
				System.out.println("Duration of Video:" + tmp.getDouble("duration"));
			}
	...
	private static void launchAppAndDownloadVideo(String AppURL, double VideoDuration, int loop) throws InterruptedException, IOException {
		driver.navigate().to(AppURL);
		Thread.sleep(5 * 1000);
		...
		if (isElementPresent(driver, videoPlayer)) {
			...
			long videoDuration = (long) (VideoDuration + 5);
